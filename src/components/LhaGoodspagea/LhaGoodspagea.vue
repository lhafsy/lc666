<template>
  <div>
    <div class="lhatop" style="position:fixed">
      <div id="topImg"><router-link to="/" ><img src="../../../img/top.png" ></router-link></div>
      <div id="topLeft"><a><router-link to="/" >主页</router-link></a></div>
      <!-- <div id="topRight"><a><router-link to="/login" >购物车</router-link></a></div> -->
    </div>
    <div class="coordinat" style="padding-top:3.2rem">
      <router-link to="/LhaGoodspageaa/HomeFurnishing" class="lhaType"><div style="background: url('../../../img/a1.png');background-size: 100%"> </div></router-link>
      <router-link to="/LhaGoodspageaa/funrniture" class="lhaType"><div style="background: url('../../../img/a2.png');background-size: 100%"></div> </router-link>
      <router-link to="/LhaGoodspageaa/Stationery" class="lhaType"><div style="background: url('../../../img/a3.png');background-size: 100%"></div> </router-link>
      <router-link to="/LhaGoodspageaa/Digital" class="lhaType"><div style="background: url('../../../img/a4.jpg');background-size: 100%"></div></router-link>
     <router-link to="/LhaGoodspageaa/Fun" class="lhaType"> <div style="background: url('../../../img/a5.png');background-size: 100%"></div></router-link>
     <router-link to="/LhaGoodspageaa/KitchenAndToilet" class="lhaType"> <div style="background: url('../../../img/a6.png');background-size: 100%"></div></router-link>
     <router-link to="/LhaGoodspageaa/DeliciousFood" class="lhaType"> <div style="background: url('../../../img/a7.png');background-size: 100%"></div></router-link>
     <router-link to="/LhaGoodspageaa/MenisWear" class="lhaType"> <div style="background: url('../../../img/a8.png');background-size: 100%"></div></router-link>
     <router-link to="/LhaGoodspageaa/Ladies" class="lhaType"> <div style="background: url('../../../img/a9.png');background-size: 100%"></div></router-link>
      <router-link to="/LhaGoodspageaa/Childrenisclothing" class="lhaType"><div style="background: url('../../../img/a10.png');background-size: 100%"></div></router-link>
      <router-link to="/LhaGoodspageaa/Shoesandbags" class="lhaType"><div style="background: url('../../../img/a11.png');background-size: 100%"></div></router-link>
      <router-link to="/LhaGoodspageaa/Accessories" class="lhaType"><div style="background: url('../../../img/a12.png');background-size: 100%"></div></router-link>
      <router-link to="/LhaGoodspageaa/a" class="lhaType"><div style="background: url('../../../img/a13.png');background-size: 100%"></div></router-link>
      <router-link to="/LhaGoodspageaa/a" class="lhaType"><div style="background: url('../../../img/a14.png');background-size: 100%"></div></router-link>
      <router-link to="/LhaGoodspageaa/a" class="lhaType"><div style="background: url('../../../img/a15.png');background-size: 100%"></div></router-link>
      <router-link to="/LhaGoodspageaa/a" class="lhaType"><div style="background: url('../../../img/a16.jpg');background-size: 100%"></div></router-link>
      <router-link to="/LhaGoodspageaa/a" class="lhaType"><div style="background: url('../../../img/a17.jpg');background-size: 100%"></div></router-link>
      <router-link to="/LhaGoodspageaa/a" class="lhaType"><div style="background: url('../../../img/a18.png');background-size: 100%"></div></router-link>
      <router-link to="/LhaGoodspageaa/a" class="lhaType"><div style="background: url('../../../img/a19.png');background-size: 100%"></div></router-link>
    </div>

  </div>

</template>

<script type="text/javascript">

  import './LhaGoodspagea.css'

  export default {
    name: 'LhaGoodspagea'
    // data: function(){
    //  return {name: 'Tom'}
    // }
  }
</script>
<style>

</style>

