...
$.extend(mggScrollImg.prototype,{
    ...
    bind_event : function(){
        ...
        self.box.bind('touchstart',function(e){
            if(e.originalEvent.targetTouches.length==1 && !self.busy){
                self.point_x = e.originalEvent.targetTouches[0].screenX;
                self.point_y = e.originalEvent.targetTouches[0].screenY;
            }
            // e.originalEvent.targetTouches[0].clientX
        }).bind('touchmove',function(e){
            if(e.originalEvent.targetTouches.length==1 && !self.busy){
                return self.move(e.originalEvent.targetTouches[0].screenX,e.originalEvent.targetTouches[0].screenY);//这里根据返回值觉得是否阻止默认touch事件
            }
